...
load_dotenv()  # 같은 폴더의 .env 파일에서 환경변수를 읽어옵니다.
...
def get_credits(uid: str) -> int:
    try:
        resp = requests.get(f"{PAYMENT_SERVER_URL}/credits/{uid}", timeout=8)
...
def deduct_credit(uid: str) -> int:
    try:
        resp = requests.post(f"{PAYMENT_SERVER_URL}/consume", json={"deviceId": uid}, timeout=8)
        resp.raise_for_status()
        return int(resp.json().get("balance", 0))
    except Exception as e:
